...
//    public static double timeForFormNewCluster = 0;
    int numWindows = 0;
//    public static int numberDispersedCluster =0 ;
...
//        System.out.println("Number of dispersed clusters = "+ numberDispersedCluster);
        
        System.out.println(" Size of dataList_set = "+ dataList_set.size());
        System.out.println(" size of dataList = "+ dataList.size());
        System.out.println(" size of eventQueue = "+ eventQueue.size());
        System.out.println(" Size of outlier list = "+ outlierList.size());
        System.out.println(" Size of pd list = "+ PD.size());
    ...
    private void addToPD(MCO o, boolean fromCluster) {
        ...
        PD.stream().forEach((inPD) -> {
            ...
            if (distance <= Constants.R) {
                ...
                Collections.sort(o.exps);

                    while (o.exps.size() > Constants.k - o.numberOfSucceeding && o.exps.size() > 0) {
                        o.exps.remove(0);
                    }
...
//                addToHashMap(o.arrivalTime,d.arrivalTime);
    ...
    private void process_event_queue(int currentTime) {
        ...
        while (x != null && x.ev <= currentTime) {
            ...
            x = eventQueue.poll();
            while (x.exps.size() > 0 && x.exps.get(0) <= currentTime) {
                x.exps.remove(0);
                if (x.exps.isEmpty()) {
                    x.ev = 0;
                    break;
